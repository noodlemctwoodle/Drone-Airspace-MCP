...
describe('osm hazards', () => {
  it('classifies hazard tags and ignores sidings, trenches and other ways', () => {
    ...
    expect(classifyHazard({ power: 'minor_line' })).toBe('minor_power_line');
    expect(classifyHazard({ power: 'tower' })).toBe('pylon');
    expect(classifyHazard({ power: 'substation' })).toBe('substation');
    expect(classifyHazard({ power: 'plant' })).toBe('power_generator');
    ...
    expect(classifyHazard({ amenity: 'kindergarten' })).toBe('kindergarten');
    expect(classifyHazard({ amenity: 'hospital' })).toBe('hospital');
    expect(classifyHazard({ amenity: 'fire_station' })).toBe('fire_station');
    expect(classifyHazard({ amenity: 'fuel' })).toBe('fuel_station');
    expect(classifyHazard({ amenity: 'grave_yard' })).toBe('cemetery');
    expect(classifyHazard({ landuse: 'cemetery' })).toBe('cemetery');
    expect(classifyHazard({ leisure: 'park' })).toBe('park');
    expect(classifyHazard({ leisure: 'pitch' })).toBeUndefined();
  ...
  it('keeps site polygons, collapses pylons and masts to points and takes school nodes as they are', () => {
    const square = [[[-2.5, 51.4], [-2.499, 51.4], [-2.499, 51.401], [-2.5, 51.401], [-2.5, 51.4]]];
    const school = normaliseHazardFeature({ geometry: { type: 'Polygon', coordinates: square }, properties: { '@id': 'w20', amenity: 'school', name: 'Test Primary School' } });
    expect(school[0]).toMatchObject({ kind: 'school', name: 'Test Primary School' });
    expect(school[0].geometry.type).toBe('Polygon');
    const node = normaliseHazardFeature({ geometry: { type: 'Point', coordinates: [-2.5, 51.4] }, properties: { amenity: 'kindergarten' } });
    expect(node[0]).toMatchObject({ kind: 'kindergarten', geometry: { type: 'Point', coordinates: [-2.5, 51.4] } });
    const pylon = normaliseHazardFeature({ geometry: { type: 'Point', coordinates: [-2.51234567, 51.4] }, properties: { power: 'tower', ref: 'ZZ123' } });
