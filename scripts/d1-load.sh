...
const c = {}; for (const t of ['zones','rights_of_way','land_restrictions','coverage','gazetteer']) c[t] = db.prepare('SELECT COUNT(*) AS n FROM '+t).get().n;
console.log(JSON.stringify(c));" "$PACK")
ACTUAL=$(npx wrangler d1 execute "$DB_NAME" --remote --yes --json --command "SELECT (SELECT COUNT(*) FROM zones) AS zones, (SELECT COUNT(*) FROM rights_of_way) AS rights_of_way, (SELECT COUNT(*) FROM land_restrictions) AS land_restrictions, (SELECT COUNT(*) FROM coverage) AS coverage, (SELECT COUNT(*) FROM gazetteer) AS gazetteer, (SELECT COUNT(*) FROM zones_rtree) AS zones_rtree, (SELECT COUNT(*) FROM rights_of_way_rtree) AS prow_rtree" 2>/dev/null | node -e "
...
const bad = Object.keys(e).filter(k => e[k] !== a[k]).concat(a.zones_rtree !== a.zones ? ['zones_rtree'] : [], a.prow_rtree !== a.rights_of_way ? ['prow_rtree'] : []);
