...
export function createCheckTakeoffSiteHandler(deps: HandlerDependencies): ToolHandler {
  return async (args) => {
    ...
    const hazards = await pack.hazardsNear(loc.lon, loc.lat, hazardRadiusM, 12);
    ...
    return respond(format, data, () => {
      const sections: ReportSection[] = [
        ...
        { title: `Ground hazards within ${formatDistance(hazardRadiusM)} (${hazards.length})`, lines: hazards.map(renderHazard) },
